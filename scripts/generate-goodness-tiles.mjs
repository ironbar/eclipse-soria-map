...
const outputDir = path.join(rootDir, "public", "goodness");
...
function colorForGoodness(value) {
  if (!Number.isFinite(value)) {
    return [0, 0, 0, 0];
  }

  if (value > 7) {
    return [204, 55, 67, 238];
  }
...
async function writeTile(z, x, y, grid, stats) {
  ...
  for (let row = 0; row < analysisTileSize; row++) {
    for (let column = 0; column < analysisTileSize; column++) {
      ...
      const [red, green, blue, alpha] = colorForGoodness(value);
...
async function main() {
    ...
    `${JSON.stringify(
      {
        ...
        maxDistanceMeters,
        thresholds: {
          good: "0 to 5 degrees, green gradient",
          acceptable: "5 to 7 degrees, yellow gradient",
          poor: "above 7 degrees, red",
        },
  ...
  console.log(`Goodness source horizon range: ${stats.min.toFixed(2)}° - ${stats.max.toFixed(2)}°.`);
